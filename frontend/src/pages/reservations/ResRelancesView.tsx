...
function buildRelances(rows: ReservationRow[]): RelanceItem[] {
  ...
  rows.forEach((r) => {
    ...
    // Option expirant dans moins de 48h
    if (r.status === 'hold' && daysToArrival <= 2) {
      items.push({
        id: `${r.id}-hold`,
        row: r,
        type: 'option_expire',
        urgency: daysToArrival <= 0 ? 'high' : daysToArrival <= 1 ? 'high' : 'medium',
    ...
    // Pending sans réponse depuis plus de 24h
    if (r.status === 'pending') {
    ...
    // Acompte manquant (confirmé mais pas encore payé, arrivée dans < 7 jours)
    if (r.status === 'confirmed' && r.payment_status === 'pending' && balance > 0 && daysToArrival <= 7 && daysToArrival > 0) {
      items.push({
        id: `${r.id}-acompte`,
        row: r,
        type: 'acompte_manquant',
  ...
  // Trier : high > medium > low, puis par date d'arrivée
                ...
                <div key={item.id} className={cn('bg-white rounded-2xl ring-1 px-5 py-4 flex items-start gap-4', cfg.ring)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <span className="font-mono text-[11.5px] font-semibold text-violet-700">
