...
export const RatePlansPage: React.FC<RatePlansPageProps> = ({ onNavigate }) => {
  const {
    roomTypes, loadData, deleteRatePlan, toggleRatePlanActive, openRatePanel,
  ...
  function handleDeletePlan(roomTypeId: string, plan: RatePlanData) {
    ...
    audit({
      ...
      module: 'rms_revenue',
      detail: `${plan.planName} (${plan.planCode})`,
      meta: { planId: plan.planId, planCode: plan.planCode },
    });
    notify(`Plan "${plan.planName}" supprimé`);
  }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(({ room, plan }) => {
      ...
      if (filterPartner !== 'all') {
        const ids = plan.partnerIds ?? plan.distributionChannels ?? [];
        if (!ids.includes(filterPartner)) return false;
      }
      if (q && !`${plan.planName} ${plan.planCode} ${room.roomTypeName}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [rows, search, filterRoom, filterPartner]);
      ...
      <div className="w-full px-6 pt-6 pb-10 space-y-5">
        <header className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            />
            <button
              onClick={() => canWrite && openRatePanel(null)}
              disabled={!canWrite}
            ...
            <table className="w-full text-[13px]">
              <thead className="bg-slate-50/60 text-left text-[10.5px] uppercase tracking-wide text-slate-400">
                <tr>
                  ...
                  <th className="px-3 py-2.5 font-medium">Partenaires</th>
                  <th className="px-3 py-2.5 font-medium text-center w-28">Statut</th>
                  <th className="px-3 py-2.5 font-medium text-center w-24">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(({ room, plan }) => {
                  const partnerIds = plan.partnerIds ?? plan.distributionChannels ?? [];
                  return (
                    <tr key={`${room.roomTypeId}-${plan.planId}`} className="group border-t border-slate-100 hover:bg-violet-50/30 transition-colors">
                      <td className="px-5 py-2.5">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            <span className="text-[12.5px] font-semibold text-slate-900 truncate block">{plan.planName}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {partnerIds.length > 3 && (
                            <span className="text-[10px] text-slate-400">+{partnerIds.length - 3}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-center">
                        <button
                          onClick={() => toggleRatePlanActive(room.roomTypeId, plan.planId)}
                          disabled={!canWrite}
                          className={cn(
                            'inline-flex items-center gap-1 px-2 py-0.5 rounded-full ring-1 ring-inset text-[10.5px] font-semibold transition-colors',
                            plan.isActive
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => openRatePanel(plan.planId, room.roomTypeId)}
                            disabled={!canWrite}
                            title="Modifier"
                            className={cn('p-1.5 rounded-md text-violet-500', canWrite ? 'hover:bg-violet-100' : 'opacity-30 cursor-not-allowed')}
                          >
                            <Pencil className="w-3.5 h-3.5" />
                          </button>
                          <button
                            onClick={() => handleDeletePlan(room.roomTypeId, plan)}
                            disabled={!canWrite}
                            title="Supprimer"
                            className={cn('p-1.5 rounded-md text-rose-500', canWrite ? 'hover:bg-rose-50' : 'opacity-30 cursor-not-allowed')}
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
        ...
        <div className="rounded-xl ring-1 ring-violet-100 bg-violet-50/40 px-4 py-3 text-[11.5px] text-violet-800 flex items-start gap-2">
          ...
          <div>
            <strong>Édition inline :</strong> cliquez sur le crayon d'une ligne pour modifier un plan
            (mode de calcul, partenaires, dérivation, référent). Le statut (Actif / Inactif) est
            toggleable directement en cliquant sur le badge de la colonne Statut.
          </div>
        </div>
