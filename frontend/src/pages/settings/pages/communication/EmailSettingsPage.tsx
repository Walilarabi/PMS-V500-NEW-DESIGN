/**
 * FLOWTYM — Paramètres · Communication · Email hôtel.
 *
 * Config email PAR HÔTEL : provider (Gmail OAuth / Microsoft Graph / SMTP /
 * Resend), expéditeur, secrets en écriture seule, statut, test d'envoi réel.
 * Les emails partent de l'adresse de l'hôtel — jamais d'une adresse générique.
 */
import React, { useEffect, useState } from 'react';
import { Mail, Send, CheckCircle2, AlertTriangle, Loader2, Save, ShieldCheck, Info } from 'lucide-react';
...
export const EmailSettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const data = await getEmailSettings(); if (data) setS(data); await refreshSecrets(); }
  ...
  const save = async () => { setSaving(true); try { const saved = await saveEmailSettings(s); setS(saved); } finally { setSaving(false); } };
  const runTest = async () => {
    if (!testTo) return;
