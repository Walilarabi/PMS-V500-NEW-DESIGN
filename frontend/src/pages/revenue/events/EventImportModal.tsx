...
export const EventImportModal: React.FC<EventImportModalProps> = ({ open, onClose }) => {
  ...
  async function handleExcel(file: File) {
    ...
    try {
      ...
      const report = parseEventExcel(buf, file.name);
      const stats = bulkUpsert(report.events);
      const warn = report.warnings.length ? ` · ${report.warnings.length} avertissements` : '';
      setFeedback({
        ok: true,
        msg:
          `Feuilles traitées : ${report.sheets.join(', ')} — ` +
          `${stats.added} ajoutés · ${stats.updated} mis à jour · ` +
          `${stats.duplicates} doublons fusionnés${warn}`,
      });
    } catch (e) {
