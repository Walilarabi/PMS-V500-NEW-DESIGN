...
- ✅ **Planning v2 (legacy parity + auto-fit)** [NEW Feb 2026] : architecture multi-fichiers (`src/pages/planning/`), header style legacy avec toggle 7J/15J/Mois + Gantt/Calendrier Revenu + datepicker 12 mois, filtres (search/type/statut/canal), stats rows sticky (TO/ADR/EVT), channel colors sur les barres (palette `BOOKING/EXPEDIA/AIRBNB/DIRECT/WALKIN/PHONE`), drag&drop @dnd-kit avec optimistic locking + **ConfirmMove dialog** (modes night/total/free pour le délogement), clic cellule vide = ReservationFormModal pré-rempli (1 nuit, chambre suggérée), clic réservation = popup détails accessible, **Calendrier Revenu** mensuel avec 4 KPIs + sous-vues KPI (heatmap occupation) et Graphiques (Recharts ComposedChart), auto-fit responsive (7J=100% / 15J=adaptatif / Mois=scroll), raccourcis clavier (prev/next/zoomIn/zoomOut). Channels & events restent en `configStore` (mock) — Phase 1B = migration Supabase channels + events.
