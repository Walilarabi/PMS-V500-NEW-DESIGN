...
-- =============================================================================
-- ⚠️ ALIGNÉ SUR LE SCHÉMA RÉEL DE PRODUCTION (P0, juin 2026).
--   La production est la source de vérité. Cette RPC n'utilise QUE des colonnes
--   réellement présentes en prod :
--     * invoices : PAS de created_by (acteur NULL) ; credit_note_of pour les avoirs.
--     * payments : created_by + reservation_id directs ; method/payment_method.
--     * audit_logs (entity='reservation') : actions réelles
--       created/updated/deleted/CHECKOUT/CHECKOUT_HK_CREATED/INSERT ; payload
--       {after} (created) / {diff:{champ:[old,new]}} (updated) ; acteur = users.id.
--   La CTE déclare explicitement ses colonnes (sinon u.entry_type introuvable).
--
-- OBJECTIF : pièces jointes (Storage) + agrégateur Journal 360°
--   (communication + CRM + cycle de vie réservation + finance), filtres serveur,
--   pagination keyset. v1 (communication_timeline) conservée.
...
CREATE POLICY comm_attach_insert ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'communication-attachments' AND (storage.foldername(name))[1] = public.get_user_hotel_id()::text);
...
BEGIN
  v_hotel := public.get_user_hotel_id();
  IF v_hotel IS NULL THEN RAISE EXCEPTION 'Aucun hôtel actif'; END IF;
  DELETE FROM public.communication_attachments WHERE id = p_id AND hotel_id = v_hotel RETURNING storage_path INTO v_path;
  RETURN v_path;
END; $$;
...
-- -----------------------------------------------------------------------------
-- 4. RPC communication_timeline_v2 — agrégateur 360° (ALIGNÉ SCHÉMA RÉEL)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  WITH unified(entry_type, entry_id, occurred_at, category, channel, direction, status,
    ...
    UNION ALL
    -- (2) communication_logs (anti-jointure)
    ...
    UNION ALL
    -- (6) Cycle de vie réservation (audit_logs) — SCHÉMA RÉEL
    ...
    UNION ALL
    -- (7) Finance — paiements / remboursements (schéma réel)
    ...
    UNION ALL
    -- (8) Finance — factures / avoirs (PAS de created_by ; credit_note_of)
